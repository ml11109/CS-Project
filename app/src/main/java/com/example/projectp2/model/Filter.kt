...
data class Filter(
    var title: String = "",
    var status: String = HabitStatus.ALL,
    ...
    fun filterHabits(habits: ArrayList<Habit>): ArrayList<Habit> {
        ...
        for (habit in habits) {
            ...
            if (this.status != HabitStatus.ALL && habit.getStatus() != this.status) match = false
            if (this.category != Category.ALL && habit.category != this.category) match = false
            if (this.frequency != Frequency.ALL && habit.frequency != this.frequency) match = false
